...
| ----------------------- | ----- | ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| Frontend TypeScript/Vue | B     | `vue-tsc`, Vitest, `scripts/lint-vue.ts`, aliases, Zod IPC schemas              | More UI behaviour probes through CDP/Playwright for critical flows                           |
| Rust/Tauri IPC          | B+    | Typed commands, `error::Error`, capability file, clippy/tests, `src/schemas.ts` | Add structural check that every command shape is mirrored in `src/types.ts`/`src/schemas.ts` |
...
| Documentation           | B     | `AGENTS.md` map, docs catalogue, plans/debt ledgers, `scripts/lint-docs.ts`     | Add freshness/ownership metadata if doc volume grows                                         |
