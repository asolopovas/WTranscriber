...
use symphonia_core::codecs::{
    CODEC_TYPE_AAC, CODEC_TYPE_MP3, CODEC_TYPE_NULL, CodecParameters, CodecType,
};
...
impl ObjectDescriptor for SLDescriptor {
    fn read<B: ReadBytes>(reader: &mut B, len: u32) -> Result<Self> {
        ...
        if predefined != SLCONFIG_PREDEFINED_MP4 {
            warn!(
                "isomp4: sl descriptor predefined {} not mp4; ignoring",
                predefined
            );
            reader.ignore_bytes(u64::from(len.saturating_sub(1)))?;
        }
